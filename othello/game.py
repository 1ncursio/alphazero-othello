...
class State:
    ...
    # 패배 여부 판정
    def is_loss(self):
        return self.is_done() and self.pieces_count(self.pieces) < self.pieces_count(
            self.enemy_pieces
        )
    ...
    def is_done(self):
        return (
            self.pieces_count(self.pieces) + self.pieces_count(self.enemy_pieces) == 36
            or self.pass_end
        )
