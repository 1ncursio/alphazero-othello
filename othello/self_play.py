...
def self_play(history):
    ...
    for i in range(SP_GAME_COUNT):
        ...
        # print length of history.history
        print("history 길이:", len(history.history), end="")
    print("")
...
if __name__ == "__main__":
    ray.init(num_cpus=4)
    history = History()
    history_ref = ray.put(history)
    self_play_ids = [self_play.remote(history_ref) for _ in range(4)]
    ray.get(self_play_ids)
    history.write_data(history)
